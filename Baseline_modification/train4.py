...
class Fit:
    def __init__(self, model, loss_fn, optimizer):
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=800//100)
        # self.scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=250) # 250
        self.dev = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        model.to(self.dev)

    def loss_batch(self, xb, yb, optimizer=None, lb=None):
        prediction = self.model(xb)
        prediction=prediction.to(self.dev)

        loss = self.loss_fn(prediction, yb)
        loss = loss # regularization term 추가..
